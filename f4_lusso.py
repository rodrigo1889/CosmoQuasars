...
def lnlike_cc(theta):
    H0, m, M, beta = theta
    delta = f4tp.H(z_cc,H0,m) - H_cc
    likk = -0.5*np.dot(delta, np.dot(icov_cc,delta))
    return likk

def lnlike_sn(theta):
    H0, m, M, beta = theta
    mb_teor = f4tp.distance_modulus(z_p,H0,m) + M
    delta = mb_teor - mb_p
...
def lnlike_SDSSDR12(theta):
    ...
    delta0 = f4tp.Hoverrd(zs,H0,m,r_fid = rdfid) - Hrs
    delta1 = f4tp.DMoverrd(zs,H0,m,r_fid = rdfid) - Dmrs
...
def lnlike_SDSSDR14(theta):
    ...
    delta0 = f4tp.Hoverrd(zs,H0,m,r_fid= 147.78)
...
def lnlike_qso(theta):
    H0, m, M, beta = theta
    mu_teor = f4tp.distance_modulus(z_qso,H0,m)
...
def lnlike_qso_flux(theta):
    H0, m, M, beta = theta
    gamma = 0.702
    logdl_teor = np.log10(f4tp.D_L(z_q,H0,m))
...
def lnprior(theta):
    ...
    if 50 < H0 < 80 and 0.0 < m <= 0.9 and -30.0 < M < 0.0 and -20 < beta < 20:
        return 0.0
    return -np.inf
...
for i in range(len(H0s)):
    ...
    backend = emcee.backends.HDFBackend("Chains/f4Lusso"+H0s[i][-1]+".h5")
