...
def lnlike_BAO_ind(theta):
    H0, m, M = theta
    delta0 = lcdm.rdoverDV(0.106,H0,m) - 0.336
    delta1 = lcdm.DVoverrd(0.15,H0,m,r_fid=148.69) - 664
    delta2 = lcdm.D_M(2.4,H0,m)/lcdm.r_d(H0,m) - 36.6
    err = np.array([0.015,25,1.2])
    delta = np.array([delta0,delta1,delta2])
    likk = -0.5*np.sum(delta**2/err**2 - np.log(err**2))
    return likk
...
def lnlike_SDSSDR14(theta):
    ...
    delta1 = lcdm.Hoverrd(zs,H0,m,r_fid= 147.78) - Hrs
    delta0 = lcdm.DAoverrd(zs,H0,m,r_fid=147.78) - DAs
...
def lnprior(theta):
    ...
    if 50 < H0 < 80 and 0.1 <= m <= 0.9 and -30.0 < M < 0.0:
        return 0.0
    return -np.inf
initial = np.array([70,0.3,-19.0])
nwalkers= 12

for i in range(len(H0s)):
    print("Calculating "+H0s[i][-1])
    def lnlike(theta):
        return lnlike_base(theta) + lnlikeH0(theta,float(H0s[i][0]),float(H0s[i][1]))
    def lnprob(theta):
        lp = lnprior(theta)
    ...
    backend = emcee.backends.HDFBackend("Chains/Draft/lcdm/Base/"+H0s[i][-1]+".h5")
