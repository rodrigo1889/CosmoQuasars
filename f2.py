...
def lnlike_BAO_ind(theta):
    H0, m, b, M = theta
    delta0 = f2tp.rdoverDV(0.106,H0,m,b) - 0.336
    #delta1 = f2tp.DVoverrd(0.15,H0,m,b,r_fid=148.69) - 664
    delta2 = np.array([f2tp.D_M(2.4,H0,m,b)/f2tp.r_d(H0,m)]) - 36.6
...
def lnlike_SDSSDR12(theta):
    ...
    deltaDM = f2tp.DMoverrd(zs,H0,m,b,r_fid = rdfid) - Dmrs
    deltaH = f2tp.Hoverrd(zs,H0,m,b,r_fid = rdfid) - Hrs
...
def lnlike_SDSSDR14(theta):
    ...
    delta1 = f2tp.Hoverrd(zs,H0,m,b,r_fid= 147.78) - Hrs
    delta0 = f2tp.DAoverrd(zs,H0,m,b,r_fid=147.78) - DAs
...
for i in range(len(H0s)):
    ...
    def lnlike(theta):
        return lnlike_light(theta) + lnlikeH0(theta,float(H0s[i][0]),float(H0s[i][1]))
    def lnprob(theta):
        lp = lnprior(theta)
