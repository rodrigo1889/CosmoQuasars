...
def lnlike_6dF(theta):
    H0, m, M, b, beta = theta
    delta = f3tp.rdoverDV(0.106,H0,m,b) - 0.336
    return -0.5*(delta**2/0.015**2) - np.log(0.015**2)

def lnlike_SDSS7(theta):
    H0, m, M, b, beta = theta
    delta = f3tp.DVoverrd(0.15,H0,m,b,r_fid=148.69) - 664
    return -0.5*(delta**2/25**2) - np.log(25**2)

def lnlike_BOSSDR11(theta):
    H0, m, M, b, beta = theta
    delta = f3tp.D_M(2.4,H0,m,b)/f3tp.r_d(H0,m) - 36.6
    return -0.5*(delta**2/1.2**2) - np.log(1.2**2)

def lnlike_eBOSS(theta):
    H0, m, M, b, beta = theta
    delta = (f3tp.D_V(1.52,H0,m,b)*147.78)/f3tp.r_d(H0,m) -3843
...
def lnlike_SDSSDR12(theta):
    ...
    delta0 = f3tp.Hoverrd(zs,H0,m,b,r_fid = rdfid) - Hrs
    delta1 = f3tp.DMoverrd(zs,H0,m,b,r_fid = rdfid) - Dmrs
...
def lnlike_SDSSDR14(theta):
    ...
    delta0 = f3tp.Hoverrd(zs,H0,m,b,r_fid= 147.78)
...
for i in range(len(H0s)):
    ...
    backend = emcee.backends.HDFBackend("Chains/f3Lusso"+H0s[i][-1]+".h5")
