...
def lnlike_6dF(theta):
    m, beta, gamma = theta
    delta = f4tp.rdoverDV(z_rdv,H0,m) - rdv
    return -0.5*np.sum(delta**2/erdv**2) + np.log(erdv**2)
def lnlike_SDSS7(theta):
    m, beta, gamma = theta
    delta = f4tp.DVoverrd(z_dv,H0,m,r_fid=r_fiddr7) - dv
    return -0.5*np.sum(delta**2/edv**2) + np.log(edv**2)
def lnlike_BAO2(theta):
    m, beta, gamma = theta
    delta = np.array([f4tp.DVoverrd(z_dv,H0,m,r_fid=r_fiddr7) - dv,f4tp.rdoverDV(z_rdv,H0,m) - rdv])
    err = np.array([edv,erdv])
    return -0.5*np.sum((delta**2/edv**2) + np.log(edv**2))
def lnlike_BAO1(theta):
    m, beta, gamma = theta
    deltadm = f4tp.DMoverrd(z_dm,H0,m,r_fid=rfiddr12) - dm
    deltah = f4tp.Hoverrd(z_h,H0,m,r_fid=rfiddr12) - h
...
def lnprior(theta):
    m, beta, gamma = theta
    if 0.1 <= m <= 0.9 and 0.1 <= beta <= 2.5 and 0.0 < gamma <= 1.0:
        return 0.0
    return -np.inf
def lnprob(theta):
    lp = lnprior(theta)
...
for i in range(len(priors)):
    ...
    backend = emcee.backends.HDFBackend("Chains/Final/f3/Lusso"+priors[i][-1]+".h5")
