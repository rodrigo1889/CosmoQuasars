...
def lnlike_sn(theta):
    m, b, beta, gamma = theta
    mb_teor = f2tp.distance_modulus(z_p,H0,m,w) + M
    delta = mb_teor - mb_p
...
def lnlike_6dF(theta):
    m, b, beta, gamma = theta
    delta = f2tp.rdoverDV(z_rdv,H0,m,w) - rdv
    return -0.5*np.sum(delta**2/erdv**2) + np.log(erdv**2)
def lnlike_SDSS7(theta):
    m, b, beta, gamma = theta
    delta = f2tp.DVoverrd(z_dv,H0,m,w,r_fid=r_fiddr7) - dv
    return -0.5*np.sum(delta**2/edv**2) + np.log(edv**2)
def lnlike_BAO2(theta):
    m, b, beta, gamma = theta
    delta = np.array([f2tp.DVoverrd(z_dv,H0,m,w,r_fid=r_fiddr7) - dv,f2tp.rdoverDV(z_rdv,H0,m,w) - rdv])
    err = np.array([edv,erdv])
    return -0.5*np.sum((delta**2/edv**2) + np.log(edv**2))
def lnlike_BAO1(theta):
    m, b, beta, gamma = theta
    deltadm = f2tp.DMoverrd(z_dm,H0,m,w,r_fid=rfiddr12) - dm
    deltah = f2tp.Hoverrd(z_h,H0,m,w,r_fid=rfiddr12) - h
...
def lnlike_qso(theta):
    m, b, beta, gamma = theta
    mu_teor = f2tp.distance_modulus(z_qso,H0,m,w)
...
def lnlike_qso_flux(theta):
    m, b, beta, gamma = theta
    logdl_teor = np.log10(f2tp.D_L(z_q,H0,m,w))
