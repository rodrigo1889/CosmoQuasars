...
def lnlike_sn(theta):
    ...
    delta = mb_teor - mb_p
    likk = -0.5*np.dot(delta,np.dot(icov_pantheon,delta)) - np.log(S/(2*np.pi))
    return likk

def lnlike_6dF(theta):
    m, b, beta, gamma = theta
    delta = f1tp.rdoverDV(z_rdv,H0,m,b) - rdv
    return -0.5*np.sum(delta**2/erdv**2) + np.log(erdv**2)
def lnlike_SDSS7(theta):
    m, b, beta, gamma = theta
    delta = f1tp.DVoverrd(z_dv,H0,m,b,r_fid=r_fiddr7) - dv
    return -0.5*np.sum(delta**2/edv**2) + np.log(edv**2)
def lnlike_BAO2(theta):
    m, b, beta, gamma = theta
    delta = np.array([f1tp.DVoverrd(z_dv,H0,m,b,r_fid=r_fiddr7) - dv,f1tp.rdoverDV(z_rdv,H0,m,b) - rdv])
    err = np.array([edv,erdv])
    return -0.5*np.sum((delta**2/edv**2) + np.log(edv**2))
def lnlike_BAO1(theta):
    m, b, beta, gamma = theta
    deltadm = f1tp.DMoverrd(z_dm,H0,m,b,r_fid=rfiddr12) - dm
    deltah = f1tp.Hoverrd(z_h,H0,m,b,r_fid=rfiddr12) - h
...
def lnprior(theta):
    m, b, beta, gamma = theta
    if 0.1 <= m <= 0.9 and -2.0 <= b <= 1.0 and 0.1 <= beta <= 2.5 and 0.0 < gamma <= 1.0:
        return 0.0
    return -np.inf
def lnprob(theta):
    lp = lnprior(theta)
