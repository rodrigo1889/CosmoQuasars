...
def f3(hub,z,m,b):
    return hub**2 - m*(1+z)**3 -(((1-m)/((2*(1/b)+1)*np.exp(-(1/b))-1))*((1+2*(1/b)*hub**2)*np.exp(-(1/b)*hub**2)-1))
def H(z,H0,m,b): #Same units as H0 (km/s/Mpc)
    #return H0*np.sqrt(optimize.root(f1,x0=10,args=(z,b,m,)).x[0])
    return H0*optimize.fsolve(f3,x0=50,args=(z,m,b,))[0]
H = np.vectorize(H)
def E(z,m,b): #without units, is the inverse of E = H/H0
    return 1/(optimize.fsolve(f3,x0=70,args=(z,m,b,)))
